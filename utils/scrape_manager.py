...
class ScrapeConfig:
    ...
    headless:          bool  = False
    allowed_langs:     list  = None   # None=不过滤, ['en','zh']=只保留英文和中文
    must_keywords:     list  = None   # AND 逻辑：JD 必须包含所有关键词
    any_keywords:      list  = None   # OR  逻辑：JD 包含任意一个关键词即保留
...
class ScrapeManager:
    ...
    def run(
            ...
            for line in self._proc.stdout:
                ...
                if line.startswith("LOG:"):
                    ...
                    try:
                        ...
                        allowed, lang_code = is_allowed(jd_text, config.allowed_langs)
                        job["lang"] = lang_code

                        if not allowed:
                            result.skipped += 1
                            log(f"  ⏭ 跳过({get_lang_display(lang_code)}): {job['title']} @ {job['company']}")
                            continue

                        # 关键词过滤
                        full_text = (job.get("title","") + " " + job.get("description","")).lower()
                        if config.must_keywords:
                            if not all(k.strip().lower() in full_text for k in config.must_keywords if k.strip()):
                                result.skipped += 1
                                log(f"  ⏭ 跳过(缺少必要关键词): {job['title']} @ {job['company']}")
                                continue
                        if config.any_keywords:
                            if not any(k.strip().lower() in full_text for k in config.any_keywords if k.strip()):
                                result.skipped += 1
                                log(f"  ⏭ 跳过(无匹配关键词): {job['title']} @ {job['company']}")
                                continue
        ...
        result.done = True
        log(f"\n🎉 完成！新增 {result.saved} 条 | 跳过 {result.skipped} 条 | 错误 {result.errors} 条")
        return result
